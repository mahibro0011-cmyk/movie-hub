...
// on the deployment. If the env var IS set (even to a different value), it wins.
// Keep this in sync with the same constant in api/webhook.js.
const DEFAULT_ADMIN_ID = '5697990319';

function verifyAdmin(initData) {
  const tgUser = verifyInitData(initData);
  if (!tgUser) return null;
  const adminId = String(process.env.ADMIN_TELEGRAM_ID || DEFAULT_ADMIN_ID);
  if (String(tgUser.id) !== adminId) return null;
  return tgUser;
}
